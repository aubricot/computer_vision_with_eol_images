...
# Last updated 14 Sep 2025 by K Wolcott
...
# For running inference
# Modified from TF Hub https://www.tensorflow.org/hub/tutorials/object_detection
def run_detector_tf(detection_graph, image_url, outfpath, filters, label_map, max_boxes, min_score, category_index):
...
def denormalize_coords(crops):
    crops['xmin'] = (crops['xmin'] * crops['im_width']).round(2)
    crops['ymin'] = (crops['ymin'] * crops['im_height']).round(2)
    crops['xmax'] = (crops['xmax'] * crops['im_width']).round(2)
    crops['ymax'] = (crops['ymax'] * crops['im_height']).round(2)
    return crops
...
def draw_box_on_image(df, i, img, tags):
    # Get box coordinates
    xmin = int(round(df['xmin'][i]))
    ymin = int(round(df['ymin'][i]))
    xmax = int(round(df['xmax'][i]))
    ymax = int(round(df['ymax'][i]))
    ...
    image_wbox = cv2.putText(img, tag, ((xmin+buffer), (ymin-buffer)), cv2.FONT_HERSHEY_SIMPLEX, fontsize, box_col, 2, cv2.LINE_AA)

    # Draw box label on image
    image_wbox = cv2.rectangle(img, ((xmin-buffer), (ymin-buffer)), ((xmax+buffer), (ymax+buffer)), box_col, thickness)
...
# Run inference
def run_inference(df, detector, outfpath, label_map, filter, export_results, display_image, run_detector_tf, category_index,
                  max_boxes=10, min_score=0.6, start=0, stop=20, cutoff=20,
    ...
    for i, row in enumerate(df.iloc[start:stop].iterrows()):
        try:
            ...
            #image_url = df['eolMediaURL'][i]
            image_wboxes, result, im_h, im_w = run_detector_tf(
                detector, image_url, outfpath, filter, label_map, max_boxes, min_score, category_index
            )
            ...
            print(f'Error processing image at index {i} (URL: {df["url"][i]}): {e}')
