...
# Run inference
def run_inference(df, detector, outfpath, label_map, filter, export_results, display_image, run_detector_tf,
    ...
    for i, row in enumerate(df.iloc[start:stop].iterrows()):
        try:
            ...
            image_wboxes, result, im_h, im_w = run_detector_tf(
                detector, image_url, outfpath, filter, label_map, max_boxes, min_score
            )
